...
shareuvm(pde_t* pgdir, uint sz){
  ...
  //lcr3(V2P(d));
  lcr3(V2P(pgdir)); //페이지 테이블 재설치및 TLB flush
  //cprintf("finish shareuvm\n");
  return d;
bad:
  freevm(d);
  return 0;
...
void Cow_handler(void){
  char* va=(char*)PGROUNDDOWN(rcr2()); //안하면 mappages error
  struct proc* curproc=myproc();
  pte_t* pte;
  uint pa, flags;
  char* mem;
  ...
  if(sharecnt==1){ //공유되지 않는 경우
    ...
    *pte= *pte & ~PTE_P;
    ...
    if(mappages(curproc->pgdir, (void*)va, PGSIZE, V2P(mem), flags) < 0){
      ...
      //mappages(curproc->pgdir, (void*)va , PGSIZE, pa, flags & ~PTE_W);
      kfree(mem);
      goto bad;
    }
    kfree((char*)P2V(pa));
