...
void print_ptable(){
  for(struct proc* thread=ptable.thread; thread<&ptable.thread[NPROC]; thread++){
    if(thread->state!=UNUSED){
      cprintf("pid:%d, tid:%d, state:%d parent:%d pgdir:%d kstack:%d chan:%d\n", thread->pid, thread->tid, thread->state, thread->parent->pid, thread->pgdir, thread->kstack, thread->chan);
    }
  }
}

//--Thread Function
//스레드의 시작함수가 종료되면 호출되는 함수
void thread_wait(){
...
found:
  ...
  if(flag==0){
    sz=PGROUNDUP(sz);
    if((sz=allocuvm(curthread->pgdir, sz, sz+PGSIZE))==0){
      kfree(t->kstack);
      t->kstack=0;
      t->state=UNUSED;
      release(&ptable.lock);
  ...
  //Copy same part of parent process
...
//스레드를 종료시키고 return값을 value에 저장한다.
...
wait(void)
{
  ...
  for(;;){
    ...
    // if(wakeup_iter){
    //   cprintf("havekids:%d, pid:%d, flag:%d\n", havekids, pid, flag);
    //   wakeup_iter=0;
    // }
    if(flag && pid!=0 && havekids){ //zombie process
      //cprintf("free zombie pid:%d\n", pid);
      int free_pd=0;
      for(p=ptable.thread;p<&ptable.thread[NPROC];p++){
        if(p->parent!=curproc) continue;
        if(p->state==ZOMBIE){
